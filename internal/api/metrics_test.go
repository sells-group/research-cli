...
func TestMetrics_Success(t *testing.T) {
	...
	st.EXPECT().SummarizeRuns(mock.Anything, mock.Anything).Return(&store.RunSummary{}, nil)
	st.EXPECT().CountDLQ(mock.Anything).Return(0, nil)

	collector := monitoring.NewCollector(st, nil)
	h := NewHandlers(&config.Config{}, st, nil, collector, nil)
...
func TestMetrics_CustomLookback(t *testing.T) {
	...
	h := NewHandlers(cfg, st, nil, collector, nil)
...
func TestMetrics_NilConfig(t *testing.T) {
	...
	h := NewHandlers(nil, st, nil, collector, nil)
...
func TestMetrics_CollectError(t *testing.T) {
	st := mocks.NewMockStore(t)
	st.EXPECT().SummarizeRuns(mock.Anything, mock.Anything).Return(nil, eris.New("db connection lost"))
