-- 081: Add blog & content intelligence columns to PE intelligence materialized view
-- Adds 10 new blog intelligence extraction answers from Category G questions.
...
SELECT
    ...
    -- Blog & Content Intelligence answers (Category G)
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_investment_themes') AS investment_themes,
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_market_views') AS market_views,
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_target_profile_signals') AS target_profile_signals,
    (SELECT pa.value FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_deal_announcements') AS deal_announcements,
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_fundraise_signals') AS fundraise_signals,
    (SELECT pa.value FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_portfolio_updates') AS portfolio_updates,
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_hiring_expansion') AS hiring_expansion,
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_thought_leadership') AS thought_leadership,
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_content_recency') AS content_recency,
    (SELECT pa.value #>> '{}' FROM fed_data.pe_answers pa WHERE pa.pe_firm_id = pf.pe_firm_id AND pa.question_key = 'pe_competitive_intel') AS competitive_intel,
