...
func TestDams_Metadata(t *testing.T) {
	s := &HIFLDDams{}
	assert.Equal(t, "hifld_dams", s.Name())
...
func TestDams_ShouldRun(t *testing.T) {
	s := &HIFLDDams{}
	now := fixedNow()
...
func TestDams_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/dams.json")
	...
	s := &HIFLDDams{baseURL: srv.URL + "/query"}
...
func TestDams_NullGeometry(t *testing.T) {
	data := []byte(`{
		"features": [
...
func TestDams_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/dams.json")
	require.NoError(t, err)
...
func TestDams_QueryError(t *testing.T) {
	// Point at a nonexistent server to trigger a download error.
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()

	s := &HIFLDDams{baseURL: "http://127.0.0.1:1/query"}
	f := fetcher.NewHTTPFetcher(fetcher.HTTPOptions{MaxRetries: 0})
	_, err = s.Sync(context.Background(), mock, f, t.TempDir())
	require.Error(t, err)
