...
func TestHospitals_Metadata(t *testing.T) {
	s := &HIFLDHospitals{}
	assert.Equal(t, "hifld_hospitals", s.Name())
...
func TestHospitals_ShouldRun(t *testing.T) {
	s := &HIFLDHospitals{}
	now := fixedNow()
...
func TestHospitals_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/hospitals.json")
	...
	s := &HIFLDHospitals{baseURL: srv.URL + "/query"}
...
func TestHospitals_NullGeometry(t *testing.T) {
	data := []byte(`{
		"features": [
...
func TestHospitals_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/hospitals.json")
	require.NoError(t, err)
...
func TestHospitals_EmptyResponse(t *testing.T) {
...
func TestHospitals_QueryError(t *testing.T) {
	// Point at a nonexistent server to trigger a download error.
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()

	s := &HIFLDHospitals{baseURL: "http://127.0.0.1:1/query"}
	f := fetcher.NewHTTPFetcher(fetcher.HTTPOptions{MaxRetries: 0})
