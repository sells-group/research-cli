...
func TestBridges_Metadata(t *testing.T) {
	s := &HIFLDBridges{}
	assert.Equal(t, "hifld_bridges", s.Name())
...
func TestBridges_ShouldRun(t *testing.T) {
	s := &HIFLDBridges{}
	now := fixedNow()
...
func TestBridges_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/bridges.json")
	...
	s := &HIFLDBridges{baseURL: srv.URL + "/query"}
...
func TestBridges_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/bridges.json")
	require.NoError(t, err)
...
func TestBridges_EmptyResponse(t *testing.T) {
...
func TestBridges_QueryError(t *testing.T) {
	// Point at a nonexistent server to trigger a download error.
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()

	s := &HIFLDBridges{baseURL: "http://127.0.0.1:1/query"}
	f := fetcher.NewHTTPFetcher(fetcher.HTTPOptions{MaxRetries: 0})
