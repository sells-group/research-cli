...
func TestCemeteries_Metadata(t *testing.T) {
	s := &HIFLDCemeteries{}
	assert.Equal(t, "hifld_cemeteries", s.Name())
...
func TestCemeteries_ShouldRun(t *testing.T) {
	s := &HIFLDCemeteries{}
	now := fixedNow()
...
func TestCemeteries_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/cemeteries.json")
	...
	s := &HIFLDCemeteries{baseURL: srv.URL + "/query"}
...
func TestCemeteries_NullGeometry(t *testing.T) {
	data := []byte(`{
		"features": [
...
func TestCemeteries_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/cemeteries.json")
	require.NoError(t, err)
...
func TestCemeteries_EmptyResponse(t *testing.T) {
...
func TestCemeteries_QueryError(t *testing.T) {
	// Point at a nonexistent server to trigger a download error.
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()

	s := &HIFLDCemeteries{baseURL: "http://127.0.0.1:1/query"}
	f := fetcher.NewHTTPFetcher(fetcher.HTTPOptions{MaxRetries: 0})
