...
func TestSubstations_Metadata(t *testing.T) {
	s := &HIFLDSubstations{}
	assert.Equal(t, "hifld_substations", s.Name())
...
func TestSubstations_ShouldRun(t *testing.T) {
	s := &HIFLDSubstations{}
	now := fixedNow()
...
func TestSubstations_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/substations.json")
	...
	s := &HIFLDSubstations{baseURL: srv.URL + "/query"}
...
func TestSubstations_NullGeometry(t *testing.T) {
	data := []byte(`{
		"features": [
...
func TestSubstations_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/substations.json")
	require.NoError(t, err)
...
func TestSubstations_QueryError(t *testing.T) {
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()
...
func TestSubstations_EmptyResponse(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
