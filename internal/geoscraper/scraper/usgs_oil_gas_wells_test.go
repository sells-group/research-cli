...
func TestOilGasWells_Metadata(t *testing.T) {
	s := &USGSOilGasWells{}
	assert.Equal(t, "usgs_oil_gas_wells", s.Name())
...
func TestOilGasWells_ShouldRun(t *testing.T) {
	s := &USGSOilGasWells{}
	now := fixedNow()
...
func TestOilGasWells_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/oil_gas_wells.json")
	...
	s := &USGSOilGasWells{baseURL: srv.URL + "/query"}
...
func TestOilGasWells_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/oil_gas_wells.json")
	require.NoError(t, err)
...
func TestOilGasWells_EmptyResponse(t *testing.T) {
...
func TestOilGasWells_QueryError(t *testing.T) {
	// Point at a nonexistent server to trigger a download error.
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()

	s := &USGSOilGasWells{baseURL: "http://127.0.0.1:1/query"}
	f := fetcher.NewHTTPFetcher(fetcher.HTTPOptions{MaxRetries: 0})
