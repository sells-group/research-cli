...
type HIFLDHistoricPlaces struct {
	baseURL string // override for testing; empty uses default HIFLD endpoint
}

// Name implements GeoScraper.
func (h *HIFLDHistoricPlaces) Name() string { return "hifld_historic_places" }

// Table implements GeoScraper.
func (h *HIFLDHistoricPlaces) Table() string { return "geo.infrastructure" }

// Category implements GeoScraper.
func (h *HIFLDHistoricPlaces) Category() geoscraper.Category { return geoscraper.National }

// Cadence implements GeoScraper.
func (h *HIFLDHistoricPlaces) Cadence() geoscraper.Cadence { return geoscraper.Annual }

// ShouldRun implements GeoScraper.
func (h *HIFLDHistoricPlaces) ShouldRun(now time.Time, lastSync *time.Time) bool {
	return hifldAnnualShouldRun(now, lastSync)
}

// Sync implements GeoScraper.
func (h *HIFLDHistoricPlaces) Sync(ctx context.Context, pool db.Pool, f fetcher.Fetcher, _ string) (*geoscraper.SyncResult, error) {
	log := zap.L().With(zap.String("scraper", h.Name()))
	log.Info("starting historic_places sync")
	...
	flush := func() error {
		...
		if err != nil {
			return eris.Wrap(err, "historic_places: upsert batch")
		}
		totalRows += n
		batch = batch[:0]
		return nil
	...
	err := arcgis.QueryAll(ctx, f, arcgis.QueryConfig{
		...
		for _, feat := range features {
			...
			row := []any{
				...
				hifldSource,
				sourceID,
				hifldProperties(feat.Attributes, historicPlaceExclude),
			}
	...
	if err != nil {
		return nil, eris.Wrap(err, "historic_places: query arcgis")
	}
	...
	log.Info("historic_places sync complete", zap.Int64("rows", totalRows))
