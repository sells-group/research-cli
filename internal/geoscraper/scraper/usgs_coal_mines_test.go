...
func TestCoalMines_Metadata(t *testing.T) {
	s := &USGSCoalMines{}
	assert.Equal(t, "usgs_coal_mines", s.Name())
...
func TestCoalMines_ShouldRun(t *testing.T) {
	s := &USGSCoalMines{}
	now := fixedNow()
...
func TestCoalMines_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/coal_mines.json")
	...
	s := &USGSCoalMines{baseURL: srv.URL + "/query"}
...
func TestCoalMines_NullGeometry(t *testing.T) {
	data := []byte(`{
		"features": [
			{"attributes": {"OBJECTID": 1, "MSHA_ID": "M001", "MINE_NAME": "Good"}, "geometry": {"x": -95.0, "y": 30.0}},
			{"attributes": {"OBJECTID": 2, "MSHA_ID": "M002", "MINE_NAME": "Bad"}, "geometry": null}
		],
...
func TestCoalMines_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/coal_mines.json")
	require.NoError(t, err)
...
func TestCoalMines_EmptyResponse(t *testing.T) {
...
func TestCoalMines_QueryError(t *testing.T) {
	// Point at a nonexistent server to trigger a download error.
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()

	s := &USGSCoalMines{baseURL: "http://127.0.0.1:1/query"}
	f := fetcher.NewHTTPFetcher(fetcher.HTTPOptions{MaxRetries: 0})
