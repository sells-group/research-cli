...
func TestAirports_Metadata(t *testing.T) {
	s := &HIFLDAirports{}
	assert.Equal(t, "hifld_airports", s.Name())
...
func TestAirports_ShouldRun(t *testing.T) {
	s := &HIFLDAirports{}
	now := fixedNow()
...
func TestAirports_Sync(t *testing.T) {
	data, err := os.ReadFile("testdata/airports.json")
	...
	s := &HIFLDAirports{baseURL: srv.URL + "/query"}
...
func TestAirports_NullGeometry(t *testing.T) {
	data := []byte(`{
		"features": [
...
func TestAirports_UpsertError(t *testing.T) {
	data, err := os.ReadFile("testdata/airports.json")
	require.NoError(t, err)
...
func TestAirports_EmptyResponse(t *testing.T) {
...
func TestAirports_QueryError(t *testing.T) {
	// Point at a nonexistent server to trigger a download error.
	mock, err := pgxmock.NewPool()
	require.NoError(t, err)
	defer mock.Close()

	s := &HIFLDAirports{baseURL: "http://127.0.0.1:1/query"}
	f := fetcher.NewHTTPFetcher(fetcher.HTTPOptions{MaxRetries: 0})
