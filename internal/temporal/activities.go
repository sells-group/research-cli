...
func (a *Activities) DownloadFOIAZIP(ctx context.Context, params DownloadParams) (string, error) {
	...
	if err := os.MkdirAll(params.TempDir, 0o750); err != nil {
		return "", eris.Wrap(err, "temporal: create temp dir")
	}
	...
	if _, err := a.Fetcher.DownloadToFile(ctx, params.ZIPURL, zipPath); err != nil {
		return "", eris.Wrapf(err, "temporal: download ZIP %s", params.Name)
	}
...
func (a *Activities) ExtractAndMapPDFs(ctx context.Context, params ExtractParams) ([]DocMappingResult, error) {
	...
	if err := os.MkdirAll(extractDir, 0o750); err != nil {
		return nil, eris.Wrap(err, "temporal: create extract dir")
	}

	extractedFiles, err := fetcher.ExtractZIP(params.ZIPPath, extractDir)
	if err != nil {
		return nil, eris.Wrapf(err, "temporal: extract ZIP %s", params.Name)
	}
	...
	if mappingPath == "" {
		return nil, eris.Errorf("temporal: mapping CSV not found in ZIP %s", params.Name)
	}
	...
	if err != nil {
		return nil, eris.Wrapf(err, "temporal: open mapping CSV %s", params.Name)
	}
	...
	if err := <-errCh; err != nil {
		return nil, eris.Wrapf(err, "temporal: parse mapping CSV %s", params.Name)
	}
...
func (a *Activities) ProcessPDFViaDocling(ctx context.Context, params ProcessPDFParams) (*ProcessPDFResult, error) {
	...
	if err != nil {
		return nil, eris.Wrapf(err, "temporal: read PDF %s", params.PDFPath)
	}

	doc, err := a.Docling.Convert(ctx, pdfData, docling.ConvertOpts{})
	if err != nil {
		return nil, eris.Wrapf(err, "temporal: docling convert CRD %d doc %s", params.CRDNumber, params.DocID)
	}
...
func (a *Activities) UpsertDocumentBatch(ctx context.Context, params UpsertParams) (int64, error) {
	...
	if err != nil {
		return 0, eris.Wrapf(err, "temporal: upsert documents to %s", params.Table)
	}
	return n, nil
}
...
func (a *Activities) UpsertSectionBatch(ctx context.Context, params UpsertSectionParams) (int64, error) {
	...
	if err != nil {
		return 0, eris.Wrapf(err, "temporal: upsert sections to %s", params.Table)
	}
	return n, nil
}
