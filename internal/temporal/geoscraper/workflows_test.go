...
func TestScrapeWorkflow_PartialFailure(t *testing.T) {
	...
	env.OnWorkflow(ScrapeSingleWorkflow, mock.Anything, sdk.SyncItemParams{Name: "hifld"}).
		Return(&ScrapeSingleResult{RowsSynced: 500}, nil)
	env.OnWorkflow(ScrapeSingleWorkflow, mock.Anything, sdk.SyncItemParams{Name: "fema_flood"}).
...
func TestScrapeSingleWorkflow_Success(t *testing.T) {
	...
	env.ExecuteWorkflow(ScrapeSingleWorkflow, ScrapeSingleParams{Name: "hifld"})
	require.True(t, env.IsWorkflowCompleted())
	require.NoError(t, env.GetWorkflowError())
