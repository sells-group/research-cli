...
// CreateAllStateTables prepares templates, creates parent tables, and creates
// per-state child tables for all requested states.
func (a *Activities) CreateAllStateTables(ctx context.Context, params CreateStateTablesParams) error {
	products := resolveProducts(params.Tables)

	// Prepare template tables and parent tables.
	if err := tiger.PrepareTemplates(ctx, a.pool); err != nil {
		return eris.Wrap(err, "prepare templates")
	}
	if err := tiger.CreateParentTables(ctx, a.pool, products); err != nil {
		return eris.Wrap(err, "create parent tables")
	}
...
// LoadStateProducts loads all per-state and per-county products for a single state.
...
func (a *Activities) LoadStateProducts(ctx context.Context, params LoadStateProductsParams) (*LoadStateProductResult, error) {
	...
	var perState, perCounty []tiger.Product
	for _, p := range products {
		switch {
		case p.National:
			continue
	...
	// Load per-state products (PLACE, COUSUB).
	...
	if len(perCounty) > 0 {
		rows, err := tiger.LoadCountyProducts(ctx, a.pool, params.State, params.FIPS, perCounty, opts)
		if err != nil {
			return nil, eris.Wrapf(err, "load county products for %s", params.State)
		}
		totalRows += rows
	}
